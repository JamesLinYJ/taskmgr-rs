...
| 模块 | 主要职责 |
| --- | --- |
| `app.rs` | 组合根、主窗口消息循环、启动和关闭顺序、页面协调 |
| `app_controllers.rs` | 托盘、运行时统计、菜单状态和窗口模式等长期控制器 |
| `pages.rs` | 页面生命周期、激活/停用、页面菜单和消息路由 |
| `background_worker.rs` | 持久 worker、请求/完成通道和有序关闭 |
| `process_identity.rs` | 进程稳定身份、创建时间校验和已验证句柄 |
| `system_sampler.rs`, `cpu_sampler.rs`, `cpu_topology.rs` | 系统级 CPU、内存和计数采样，以及 group-aware 处理器拓扑分类 |
| `cpu_details.rs` | CPU RelationAll、WMI、PDH 与处理器功能诊断 worker 数据源 |
| `taskpage.rs` | 应用程序窗口列表、窗口操作和图标生命周期 |
| `procpage.rs` | 进程采样、排序、虚拟列表和进程操作 |
| `perfpage.rs` | 性能页状态、布局提交和离屏表面 |
| `cpupage.rs`, `cpu_layout.rs` | 独立 CPU 诊断页、详情快照提交和响应式布局 |
| `gpupage.rs`, `gpu_layout.rs`, `gpu_sampler.rs` | 多 GPU 性能页、经典布局和 DXGI/PDH 采样 |
| `netpage.rs` | 网卡快照、计数 delta、历史和图表控件 |
| `userpage.rs` | 用户会话快照、身份校验和会话操作 |
| `chart_renderer.rs`, `perf_drawing.rs`, `drawing.rs` | 绘制原语、历史数据和图表渲染 |
| `options.rs` | 注册表二进制配置、校验、规范化和保存 |
| `assets.rs`, `dialog_templates.rs`, `runtime_menu.rs` | 原生资源、模板和菜单所有权 |
| `language.rs`, `localization_*`, `localization/*.toml` | 本地化键、生成代码入口和翻译源 |
| `winutil.rs` | 小型、跨模块、语义明确的 Win32 RAII 与工具函数 |
| `build.rs` | 资源嵌入、manifest 和本地化代码生成 |
...
- 不要为每个 tick 创建线程。使用 `BackgroundWorker` 或同等生命周期清晰的长期 worker。
- 同一采样器最多允许一个在途请求。刷新期间再次请求时设置 `refresh_requested`，完成后合并为一次后续采样。
...
- 修改图标、位图、manifest 或资源名时同步更新 `build.rs`、资源常量和 `cargo:rerun-if-changed`。
